...
(function (global) {
	...
	function b642packed(str, existingPacked, existingPackedLen, bigEndianMod) {
		...
		for (i = 0; i < str.length; i += 4) {
			...
			for (j = 0; j < strPart.length - 1; j += 1) {
				...
				packed[intOffset] |= (tmpInt >>> 16 - j * 8 & 0xFF) << 8 * (shiftModifier + bigEndianMod * (byteOffset % 4));
				byteCnt += 1;
			}
		}
  ...
  */
	function packed2b64(packed, outputLength, bigEndianMod, formatOpts) {
		...
		for (i = 0; i < length; i += 3) {
			...
			triplet = (packed[i >>> 2] >>> 8 * (shiftModifier + bigEndianMod * (i % 4)) & 0xFF) << 16 | (int1 >>> 8 * (shiftModifier + bigEndianMod * ((i + 1) % 4)) & 0xFF) << 8 | int2 >>> 8 * (shiftModifier + bigEndianMod * ((i + 2) % 4)) & 0xFF;
			for (j = 0; j < 4; j += 1) {
  ...
  */
     ...
     *   length of "value"
     */
				retVal = function retVal(arr, existingBin, existingBinLen) {
  ...
  */
	var jsSHA = function jsSHA(variant, inputFormat, options) {
		...
		if ("SHA-1" === shaVariant && (1 & SUPPORTED_ALGS) !== 0) {
			...
			outputBinLen = 160;
			stateCloneFunc = function stateCloneFunc(state) {
				return state.slice();
			};
			...
			finalizeFunc = function finalizeFunc(remainder, remainderBinLen, processedBinLen, H, outputLen) {
			...
			finalizeFunc = function finalizeFunc(remainder, remainderBinLen, processedBinLen, state, outputLen) {
