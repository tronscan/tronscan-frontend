...
class Token20Detail extends React.Component {
    ...
    confirmPrivateKey = param => {
        ...
        this.setState({
            alert: ( <SweetAlert info showCancel cancelBtnText = {
                    tu("cancel")
                }
                confirmBtnText = {
                    tu("confirm")
    ...
    preBuyTokens = token => {
        ...
        if (!wallet.isOpen) {
            this.setState({
                alert: ( <SweetAlert info showConfirm = {
                        false
                    }
                    style = {
                        {
                    ...
                    ariaHidden = "true" > </i> </a > <span > {
                        tu("login_first")
                    } </span> <button className = "btn btn-danger btn-block mt-3"
                    onClick = {
                        () => {
                            this.setState({
                                alert: null
                        ...
                        tu("OK")
                    } </button> </div > </SweetAlert>
                )
            });
            return;
        } else {
            this.setState({
                alert: ( <SweetAlert showConfirm = {
                        false
                    }
                    style = {
                        {
                    ...
                    token.remaining === 0 && < span > {
                        tu("no_token_to_buy")
                    } </span>} <div className = "input-group mt-5" >
                    <input type = "number"
                    ref = {
                        ref => (this.buyAmount = ref)
                    }
                    ...
                    /> </div > <div className = "text-center mt-3 text-muted" >
                    <b> = <span ref = {
                        ref => (this.priceTRX = ref)
                    } > 0 </span> TRX </b > </div> <button className = "btn btn-danger btn-block mt-3"
                    onClick = {
                        () => {
                            this.buyTokens(token);
                        }
    ...
    buyTokens = token => {
        ...
        if (currentWallet.balance / ONE_TRX < tokenCosts) {
            this.setState({
                alert: ( <SweetAlert warning showConfirm = {
                        false
                    }
                    style = {
                        {
                    ...
                    <i className = "fa fa-times" ariaHidden = "true" > </i> </a > <span > {
            ...
            this.setState({
                alert: ( <SweetAlert warning showConfirm = {
                    ...
                    <i className = "fa fa-times" ariaHidden = "true" > </i> </a > <p className = "ml-auto buy_confirm_message" > {
    ...
    confirmTransaction = async token => {
        ...
        if (await this.submit(token)) {
            this.setState({
                alert: ( <SweetAlert success showConfirm = {
                        false
                    }
                    style = {
                        {
                    ...
                    <i className = "fa fa-times" ariaHidden = "true" > </i> </a > <h5 style = {
                        {
                            color: "black"
                        }
                    } > {
            ...
            this.setState({
                alert: ( <SweetAlert danger title = "Error"
                    onConfirm = {
                        () => this.setState({
                            alert: null
                        })
    ...
    render() {
            ...
            return ( <main className = "container header-overlap token_black mc-donalds-coin" > {
                    alert
                } {
                    loading ? ( <div className = "card" >
                        <TronLoader > {
                            ...
                            token && ( <div className = "col-sm-12" >
                                <div className = "card" >
                                <div className = "card-body" >
                                <div className = "d-flex" > {token && token.icon_url ? ( <div> {
                                            token.contract_address == CONTRACT_ADDRESS_USDT ||
                                            token.contract_address == CONTRACT_ADDRESS_WIN ||
                                            token.contract_address == CONTRACT_ADDRESS_GGC ? ( <div className = "token-img-top" >
                                ...
                                <h5 className = "card-title" > {
                                    ...
                                    token.symbol
                                }) </h5> <p className = "card-text" > {token.token_desc
                                } </p> </div> {
                            ...
                            } </ul> </div > <div className = "card-body p-0" >
                            <Switch > {
                                tabs.map(tab => ( <
                                    Route key = {
                                        tab.id
                            ...
                            } </Switch> </div > </div> { ["transfers", "holders"].indexOf(tabName) !== -1 ? ( <
                                CsvExport downloadURL = {
                                    csvurl
                                }
                                />
                            ) : (
