...
class TokenOverview extends Component {
  ...
  loadPage = async (page = 1, pageSize = 20) => {
    ...
    if (filter.name)
      result = await xhr.get("http://172.16.21.104:8080"+"/api/token?sort=-name&limit=" + pageSize + "&start=" + (page - 1) * pageSize + "&status=ico" + "&name=" + filter.name);
    else
      result = await xhr.get("http://172.16.21.104:8080"+"/api/token?sort=-name&limit=" + pageSize + "&start=" + (page - 1) * pageSize + "&status=ico");
