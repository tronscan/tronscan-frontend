...
import {API_URL, ONE_TRX, ONE_USDJ, IS_MAINNET, CONTRACT_ADDRESS_USDJ, CONTRACT_ADDRESS_JED} from "../../../constants";
        ...
        render:(text, record, index)=>{
          return (
            text ? (<div className="d-flex flex-column">
              ...
              <div>
              {
                record.contractAddress == CONTRACT_ADDRESS_USDJ &&  <span>{ ONE_USDJ.toFixed(6)} {' USD'}</span>
              }
              {
                record.contractAddress == CONTRACT_ADDRESS_JED && <span>{ (ONE_USDJ/100).toFixed(6)} {' USD'}</span>
              }
              {(record.contractAddress == CONTRACT_ADDRESS_USDJ || record.contractAddress == CONTRACT_ADDRESS_JED) ? "": "-"}
            </div>
            )
          )
        }
