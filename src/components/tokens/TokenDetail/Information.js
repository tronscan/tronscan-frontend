                ...
                token['market_info'] && <div>
                    {
                        token['market_info'].sShortName == 'TRX'? <div className="d-flex">
                            ...
                            <Link to={`/exchange/trc20?id=${token['market_info'].pairId}`}  target="_blank" className="btn btn-danger btn-sm ml-3" style={{height: '1.2rem', lineHeight: '0.6rem',display:"none" }}> {tu('token_trade')}</Link>
                        </div>: <span style={{color: '#d8d8d8'}}>-</span>
                    }
