...
import {forIn, isEqual, uniqWith, upperFirst} from "lodash";
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return <div className="event">
            <div>
              <a onClick={this.changeWapper.bind(this, index)} className="mr-2">{record.event_name}</a>
              {
                record.eventList.map(item => {
                  return (
                    <span className="e-blue" key={item.name}> {record.result_type[item.name]} <span
                      className="e-red">{item.name}</span></span>
                  )
                })
              }
            </div>
            <div id={"event-wapper" + index} className="event-wapper p-3">
              {
                record.eventList.map(item => {
                  ...
                  <span className="e-blue mr-2">{record.result_type[item.name]} <span
                    className="e-red">{item.name}</span></span>
            ...
            <div>

              {
                record.row && (
                  <span>
                  <Select defaultValue="Hex" style={{width: 80}} onChange={value => {
