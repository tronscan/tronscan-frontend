...
export default function UpdateSettingContract(props) {
    ...
    const { cost, signature, contractType, parameterValue, consume_user_resource_percent } = contract;
    ...
    return <Fragment>
        ...
        <table className="table">
            <tbody>
                ...
                {consume_user_resource_percent && <Field label="contract_enery" text={tu("contract_enery_tip")}>{tu('contract_percent')}{100 - consume_user_resource_percent}%  {tu('contract_percent_user')}{consume_user_resource_percent}% </Field>}
                {JSON.stringify(cost) !=
