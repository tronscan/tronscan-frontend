...
class Contracts extends React.Component {
  ...
  loadContracts = async (page = 1, pageSize = 20) => {
    ...
    await Client.getContracts({
      owner: this.props.filter.address,
      limit: pageSize,
  ...
  render() {
    ...
    if (!loading && contracts.length === 0) {
        if (!EmptyState) {
          return (
              <div className="p-3 text-center no-data">{tu("account_details_contracts_no")}</div>
          );
        }
        return <EmptyState/>;
      }
    ...
    return (
      <main className="container header-overlap token_black mt-4">
        {loading && (
        ...
        <div className="row contract-list">
          ...
          <div className="col-md-12 table_pos p-0">
            {/* {total ?  */}
