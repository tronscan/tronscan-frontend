...
export default class OwnerEdit extends Component {
    ...
    render() {
        ...
        const tableList = keys.map((item, index) =>
            ...
            onChange = {
                (e) => {
                    this.changeValue(index, 2, e)
                }
            }
            /> <
            a href = "javascript:;"
            className = 'cac-btn minus'
            style = {
                {
                    visibility: index === 0 && keys.length === 1 ? 'hidden' : 'visible'
                }
            }
            onClick = {
                (e) => {
                    this.removeKeysItem(index)
                }
            } > - < /a> <
            a href = "javascript:;"
            className = 'cac-btn plus'
            style = {
                {
                    visibility: index === (keys.length - 1) && keys.length < 5 ? 'visible' : 'hidden'
                }
            }
            onClick = {
                () => {
            ...
            return ( <
                ...
                <
                div className = "permission-item" > < span className = "permission-label" > {
                    tu('signature_permission')
                }: < /span> <span><Input value={permission_name} name='permission_name' placeholder={intl.formatMessage({id: "permission_name_limit"})} maxLength={32} onChange={(e)=>{this.changeValueByEvent(e)}}/ > < /span></div >
                <
                div className = "permission-item" > < span className = "permission-label" > {
                    tu('signature_threshold')
                }: < /span> <span><Input value={threshold} name='threshold' onChange={(e)=>{this.changeValueByEvent(e)}}/ > < /span></div >
                <
                ...
                tr > < td > {
                    tu('signature_key')
                } < /td><td>{tu('signature_weight')}</td > < /tr> < /
                thead > <
                tbody > {
                    tableList
                } <
