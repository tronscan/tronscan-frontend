      ...
      if (IS_MAINNET) {
        if (
          ...
          if (this.props.wallet.type === "ACCOUNT_LEDGER") {
            ...
            result = await transactionResultManager(
              unSignTransaction,
              tronWebLedger
            );
          }
        ...
        if (
          this.props.wallet.type === "ACCOUNT_TRONLINK" ||
          this.props.wallet.type === "ACCOUNT_PRIVATE_KEY"
        ) {
          let unSignTransaction;
          if (receiver === "") {
            unSignTransaction = await sunWeb.sidechain.transactionBuilder
              .freezeBalance(
                amount * ONE_TRX,
                3,
                type,
                sunWeb.sidechain.defaultAddress.base58
              )
              .catch(e => false);
          } else {
            unSignTransaction = await sunWeb.sidechain.transactionBuilder
  ...
  render() {
    ...
    return (
      <Modal
        ...
        fade={false}
        className="modal-dialog-centered _freezeContent"
      >
        <ModalHeader
          className="text-center _freezeHeader"
          toggle={this.hideModal}
        >
          {tu("freeze")}
        </ModalHeader>
        <ModalBody className="text-center _freezeBody">
          <div className="form-group">
            <div className="text-left _power">
              {tu("current_power")}:{" "}
              <span style={{ fontWeight: 800 }}>{frozenTrx / ONE_TRX}</span>
            </div>
          </div>
          <div className="form-group">
            <input
          ...
          <div className="form-group">
            ...
            >
              {resources.map((resource, index) => {
                return (
                  <option key={index} value={resource.value}>
                    {intl.formatMessage({ id: resource.label })}
                  </option>
                );
              })}
            </select>
          ...
          {Boolean(selectedResource == 0 && getcalculate) && (
            <div className="text-left d-flex align-items-center">
              <span className="col-red mr-2">
                1TRX ≈ <FormattedNumber value={oneBandwidth} />
                {tu("bandwidth")}, {tu("Expected_acquisition")} &nbsp;{" "}
                <FormattedNumber value={getcalculate} /> &nbsp;{tu("bandwidth")}{" "}
              </span>
              <Tooltip
                placement="top"
                title={tu("energy_more")}
          ...
          {Boolean(selectedResource == 1 && getcalculate) && (
            <div className="text-left d-flex align-items-center">
              <span className="col-red mr-2">
                1TRX ≈ <FormattedNumber value={oneEnergy} />
                {tu("energy")}, {tu("Expected_acquisition")} &nbsp;{" "}
                <FormattedNumber value={getcalculate} /> &nbsp;{tu("energy")}
              </span>
              <Tooltip
                placement="top"
                title={tu("bandwidth_more")}
                overlayStyle={{ maxWidth: "320px" }}
              >
                <div className="question-mark">
                  <i>?</i>
          ...
          <div className="form-check">
            ...
            <label className="form-check-label _freeze">
              {tu("token_freeze_confirm_message_0")}{" "}
              <b>
                <FormattedNumber value={amount} /> TRX
              </b>{" "}
              {t("token_freeze_confirm_message_1")}
            </label>
          </div>
          <p className="mt-3">
            <button
